...
describe("Core percentage actions", function()
  local Core = ns.Core
  it("accepts signed percentage boundaries", function()
    for _, case in ipairs({ {1, 102}, {100, 200}, {-1, 98}, {-100, 0}, {"+15", 130}, {"-15", 70} }) do
      reset(); Core.SetHP(100, 200)
      assertTrue(Core.PercentageHeal(case[1]))
      assertEq(Core.state.hp, case[2])
    end
  end)
  it("rejects out-of-range and non-finite percentages", function()
    reset(); Core.SetHP(10, 100)
    local revision, count = Core.state.rev, #Core.state.history
    for _, value in ipairs({0, 101, -101, 0 / 0, math.huge, -math.huge, "abc"}) do
      assertFalse(Core.PercentageHeal(value))
    end
    assertEq(Core.state.hp, 10)
    assertEq(Core.state.rev, revision)
    assertEq(#Core.state.history, count)
  end)
  it("heals from maximum HP and records capped restoration", function()
    reset(); Core.SetHP(10, 200)
    assertTrue(Core.PercentageHeal(25))
    assertEq(Core.state.hp, 60); assertEq(Core.state.maxHp, 200)
    Core.SetHP(190, 200)
    assertTrue(Core.PercentageHeal(25))
    assertEq(Core.state.hp, 200)
    assertEq(Core.state.history[1].applied, 10)
  end)
  it("invalid percentage leaves wounds and HP unchanged", function()
    reset(); Core.SetHP(10, 100)
    assertFalse(Core.PercentageHeal(0))
    assertEq(Core.state.hp, 10)
    assertTrue(Core.state.wounds.hit10)
  end)
  it("negative percentage records actual loss without crediting the meter", function()
    reset(); Core.SetHP(100, 200)
    assertTrue(Core.PercentageHeal("-15"))
    assertEq(Core.state.hp, 70); assertEq(Core.state.maxHp, 200)
    assertEq(Core.state.history[1].kind, "PERCENT_DAMAGE")
    assertEq(Core.state.history[1].applied, 30)
    Core.SetHP(10, 200)
    assertTrue(Core.PercentageHeal(-15))
    assertEq(Core.state.hp, 0)
    assertEq(Core.state.history[1].applied, 10)
    assertEq(Core.state.meter.dmg, 0)
  end)
  it("pet restoration uses pet maximum HP", function()
    reset(); Core.SetPetEnabled(true); Core.SetPetHP(10, 100)
    assertTrue(Core.PetPercentageHeal(50))
    assertEq(Core.state.pet.hp, 60); assertEq(Core.state.pet.maxHp, 100)
  end)
  it("pet damage records its subject and preserves the owner meter", function()
    reset(); Core.SetPetEnabled(true); Core.SetPetHP(50, 100)
    assertTrue(Core.PetPercentageHeal(-15))
    assertEq(Core.state.pet.hp, 35); assertEq(Core.state.pet.maxHp, 100)
    assertEq(Core.state.history[1].kind, "PERCENT_DAMAGE")
    assertEq(Core.state.history[1].subject, "PET")
    assertEq(Core.state.meter.dmg, 0)
  end)
end)
...
describe("UI smoke (in-game)", function()
  ...
  it("Grimoire list rebuilds at minimum/default/large sizes", function()
    ...
    local oldTab = UI.activeTab
    UI.setSidebarSection(1); UI.setTab(UI.TAB_IDS.PLAYER_GRIMOIRE)
    for i = 1, 8 do
