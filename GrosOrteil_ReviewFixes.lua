...
-- Narrow hardening layer for issues found during the post-merge review.
...
local ReviewFixes = {}
ns.ReviewFixes = ReviewFixes

-- ---------------------------------------------------------------------------
-- Realm-aware remote identity registry.
--
-- RaidPanel's protected click path keeps Blizzard unit tokens untouched. Remote
-- addon-message senders, however, normally arrive as Name-Realm. Track those
-- exact identities and refuse ambiguous short-name lookups rather than ever
-- associating one realm's state/heal response with another realm's character.
-- ---------------------------------------------------------------------------
...
ReviewFixes.IdentityKey = identityKey
ReviewFixes.ObserveIdentity = observeIdentity
ReviewFixes.UniqueObservedIdentity = uniqueObservedIdentity
...
if Popup and Popup.OnStateReceived and Popup.GetCachedState and Popup.InjectState then
  ...
  ReviewFixes._exactStateCache = exactStateCache
end

-- ---------------------------------------------------------------------------
-- Comm multipart hardening.
-- Bound work and memory before delegating to the existing assembler.
-- ---------------------------------------------------------------------------
...
ReviewFixes.ValidMultipartPayload = validMultipartPayload
ReviewFixes.PrunePartialMessages = prunePartialMessages
ReviewFixes.MAX_PARTS = MAX_PARTS

-- ---------------------------------------------------------------------------
-- Heal-response integrity.
-- Mirror pending requests so unsolicited/duplicate responses are ignored and
-- an accepted response can never credit more healing than was requested.
-- ---------------------------------------------------------------------------
...
if Heal and Heal.MarkPending and Heal.ClearPending and Heal.SendRequest and Heal.OnResponse then
  ...
  ReviewFixes._pendingHeals = pendingMirror
  ReviewFixes.FindPendingHeal = pendingLookup
end
