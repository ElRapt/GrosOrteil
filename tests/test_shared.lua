...
T.describe("Shared.GetResProfile", function()
  ...
  T.it("Shadow and Discipline priests both carry the Insanité bar (idx 2)", function()
    for _, ck in ipairs({ "SHADOWPRIEST", "DISCPRIEST" }) do
      local p = Shared.GetResProfile({ classKey = ck })
      T.assertEq(#p, 2, ck .. " should expose two resources")
      T.assertEq(p[1].label, "Points de foi")
      T.assertEq(p[2].label, "Insanité")
      T.assertEq(p[2].idx, 2)
    end
  end)
...
T.describe("Shared.WoundCap", function()
  T.it("derives the heal cap from wound flags", function()
    T.assertEq(Shared.WoundCap(nil), 1.0)
    T.assertEq(Shared.WoundCap({}), 1.0)
    T.assertEq(Shared.WoundCap({ hit25 = true }), 0.50)
    T.assertEq(Shared.WoundCap({ hit25 = true, hit10 = true }), 0.25)
    T.assertEq(Shared.WoundCap({ hit10 = true }), 0.25)  -- hit10 wins alone too
    T.assertEq(Shared.WoundCap("junk"), 1.0)
  end)
end)

T.describe("Shared.HasInsanity", function()
  T.it("is true only for the two priest specs that carry the bar", function()
    T.assertTrue(Shared.HasInsanity("SHADOWPRIEST"))
    T.assertTrue(Shared.HasInsanity("DISCPRIEST"))
  end)
  T.it("is false for every other class (and nil/garbage)", function()
    T.assertFalse(Shared.HasInsanity("PRIEST"))
    T.assertFalse(Shared.HasInsanity("WARLOCK"))
    T.assertFalse(Shared.HasInsanity("MAGE"))
    T.assertFalse(Shared.HasInsanity(nil))
    T.assertFalse(Shared.HasInsanity(123))
  end)
end)

T.describe("Shared.GetResDisplayMax", function()
  T.it("returns the fixed caps for special idx-2 resources", function()
    T.assertEq(Shared.GetResDisplayMax("WARLOCK", 2), 60)
    T.assertEq(Shared.GetResDisplayMax("SHADOWPRIEST", 2), 25)
    T.assertEq(Shared.GetResDisplayMax("DISCPRIEST", 2), 25)
    T.assertEq(Shared.GetResDisplayMax("MAGE", 2), 8)
  end)
  T.it("returns nil everywhere else", function()
    T.assertNil(Shared.GetResDisplayMax("WARLOCK", 1))
    T.assertNil(Shared.GetResDisplayMax("MAGE", 1))
    T.assertNil(Shared.GetResDisplayMax("SHAMAN", 2))
    T.assertNil(Shared.GetResDisplayMax(nil, 2))
  end)
end)

T.describe("Shared.GetResMarkerDefs", function()
  T.it("returns the threshold sets used by every display", function()
    local w = Shared.GetResMarkerDefs("WARLOCK", 2)
    T.assertEq(#w, 3)
    T.assertNear(w[1].pct, 10/60, 1e-9)
    T.assertNear(w[3].pct, 45/60, 1e-9)
    local sp = Shared.GetResMarkerDefs("SHADOWPRIEST", 2)
    T.assertEq(#sp, 4)
    T.assertNear(sp[4].pct, 1.0, 1e-9)
    local dp = Shared.GetResMarkerDefs("DISCPRIEST", 2)
    T.assertEq(#dp, 4)
    T.assertNear(dp[1].pct, 2/25, 1e-9)
    T.assertNear(dp[4].pct, 1.0, 1e-9)
    local m = Shared.GetResMarkerDefs("MAGE", 2)
    T.assertEq(#m, 2)
    T.assertNear(m[1].pct, 0.5, 1e-9)
  end)
  T.it("returns an empty list for everything else", function()
    T.assertEq(#Shared.GetResMarkerDefs("WARLOCK", 1), 0)
    T.assertEq(#Shared.GetResMarkerDefs("ROGUE", 2), 0)
    T.assertEq(#Shared.GetResMarkerDefs(nil, nil), 0)
  end)
end)

T.describe("Shared.NormalizeNameKey", function()
  T.it("strips realm, lowers case, removes spaces", function()
    T.assertEq(Shared.NormalizeNameKey("Foo-KirinTor"), "foo")
    T.assertEq(Shared.NormalizeNameKey("Foo Bar"), "foobar")
    T.assertEq(Shared.NormalizeNameKey("FOO"), "foo")
  end)
  T.it("returns nil for non-strings", function()
    T.assertNil(Shared.NormalizeNameKey(nil))
    T.assertNil(Shared.NormalizeNameKey(42))
  end)
end)

T.describe("Shared.CompareVersions", function()
  T.it("equal numeric versions compare 0", function()
    T.assertEq(Shared.CompareVersions("120001", "120001"), 0)
  end)
  T.it("higher numeric version compares 1 / lower -1", function()
    T.assertEq(Shared.CompareVersions("120002", "120001"), 1)
    T.assertEq(Shared.CompareVersions("120001", "120002"), -1)
  end)
  T.it("dotted versions compare segment-wise (1.4.10 > 1.4.9)", function()
    T.assertEq(Shared.CompareVersions("1.4.10", "1.4.9"), 1)
  end)
  T.it("missing trailing segments count as 0", function()
    T.assertEq(Shared.CompareVersions("1.4", "1.4.0"), 0)
    T.assertEq(Shared.CompareVersions("1.4.1", "1.4"), 1)
  end)
  T.it("plain numbers are accepted", function()
    T.assertEq(Shared.CompareVersions(120002, 120001), 1)
  end)
  T.it("returns nil when either side has no digits", function()
    T.assertNil(Shared.CompareVersions("abc", "1"))
    T.assertNil(Shared.CompareVersions(nil, "1"))
    T.assertNil(Shared.CompareVersions("1", {}))
  end)
end)
