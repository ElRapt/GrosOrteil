...
T.describe("Heal.FormatResponseMessage", function()
  ...
  T.it("accepted with 0 applied uses the no-effect wording", function()
    local m = Heal.FormatResponseMessage("Bob", true, 0)
    T.assertTrue(m:find("aucun effet") ~= nil)
    T.assertTrue(m:find("Bob") ~= nil)
  end)
end)

-- ── Pending-response tracking (no-answer warning) ─────────────────────────────

T.describe("Heal pending-response tracking", function()
  T.it("SendRequest marks the target pending; OnResponse clears it", function()
    _G.MOCKS.sentMessages = {}
    Heal.ClearPending("Bob")
    Heal.SendRequest("Bob", 10)
    T.assertTrue(Heal.HasPending("Bob"))
    Heal:OnResponse("Bob", true, 10)
    T.assertFalse(Heal.HasPending("Bob"))
  end)
  T.it("ExpirePending is a no-op before the timeout, fires after", function()
    Heal.MarkPending("Tim", 5, 100)
    T.assertFalse(Heal.ExpirePending("Tim", 100 + Heal.RESPONSE_TIMEOUT - 1))
    T.assertTrue(Heal.HasPending("Tim"))
    T.assertTrue(Heal.ExpirePending("Tim", 100 + Heal.RESPONSE_TIMEOUT))
    T.assertFalse(Heal.HasPending("Tim"))
  end)
  T.it("realm suffix and case share one pending slot", function()
    Heal.MarkPending("Foo-KirinTor", 5, 1)
    T.assertTrue(Heal.HasPending("foo"))
    Heal.ClearPending("FOO")
    T.assertFalse(Heal.HasPending("Foo-KirinTor"))
  end)
end)

-- ── Incoming-request throttle (anti popup spam) ───────────────────────────────

T.describe("Heal.ShouldAcceptRequest throttle", function()
  T.it("accepts the first request, rejects rapid repeats, recovers", function()
    T.assertTrue(Heal.ShouldAcceptRequest("Spammeur", 50))
    T.assertFalse(Heal.ShouldAcceptRequest("Spammeur", 50.5))
    T.assertTrue(Heal.ShouldAcceptRequest("Spammeur", 50 + Heal.REQUEST_MIN_INTERVAL))
  end)
  T.it("tracks senders independently", function()
    T.assertTrue(Heal.ShouldAcceptRequest("SoigneurA", 10))
    T.assertTrue(Heal.ShouldAcceptRequest("SoigneurB", 10))
  end)
  T.it("rejects unusable sender names", function()
    T.assertFalse(Heal.ShouldAcceptRequest(nil, 10))
  end)
end)
