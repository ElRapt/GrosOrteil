...
T.describe("Core Shaman posture", function()
  T.it("requires ≥3 elemental points to activate", function()
    reset()
    Core.SetClassKey("SHAMAN")
    Core.SetResIndex(1, 2, 20)  -- TERRE
...
T.describe("Core Shaman postures (full)", function()
  ...
  T.it("requires ≥3 of the matching element", function()
    reset()
    Core.SetClassKey("SHAMAN")
    Core.SetResIndex(2, 2, 20)  -- AIR=2, below threshold
    Core.SetShamanPosture("AIR")
    T.assertNil(Core.state.shamanPosture)
  end)
end)
