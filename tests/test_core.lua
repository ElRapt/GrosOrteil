...
T.describe("Core.PercentageHeal(75) & PercentageHeal(50) (bypass cap)", function()
  T.it("PercentageHeal(75) grants 75% of maxHp regardless of wounds", function()
    reset()
    Core.SetHP(5, 100)
    Core.PercentageHeal(75)
    T.assertEq(Core.state.hp, 80)  -- 5 + 75
  end)
  T.it("PercentageHeal(50) grants 50% of maxHp", function()
    reset()
    Core.SetHP(0, 100)
    Core.SetStabilise(true)
    Core.PercentageHeal(50)
    T.assertEq(Core.state.hp, 50)
  end)
end)
...
T.describe("Core pet healing & regen", function()
  T.it("PetPercentageHeal(75) grants 75% maxHp", function()
    reset()
    Core.SetPetEnabled(true); Core.SetPetHP(0, 20)
    Core.PetPercentageHeal(75)
    T.assertEq(Core.state.pet.hp, 15)
  end)
  T.it("PetPercentageHeal(50) grants 50% maxHp", function()
    reset()
    Core.SetPetEnabled(true); Core.SetPetHP(0, 20)
    Core.PetPercentageHeal(50)
...
T.describe("Pet operations no-op when disabled", function()
  ...
  T.it("pet percentage/restore/daily regeneration actions all no-op when disabled", function()
    setup()
    local hp = Core.state.pet.hp
    Core.PetPercentageHeal(75); Core.PetPercentageHeal(50); Core.PetRestoreHP(); Core.PetDailyRegenHP()
...
T.describe("Stabilise persistence", function()
  ...
  T.it("PercentageHeal(75) at hp=0 with stabilise revives and clears stabilise", function()
    reset()
    Core.SetHP(0, 100); Core.SetStabilise(true)
    Core.PercentageHeal(75)
  ...
  T.it("PercentageHeal(50) at hp=0 with stabilise revives and clears stabilise", function()
    reset()
    Core.SetHP(0, 100); Core.SetStabilise(true)
    Core.PercentageHeal(50)
    T.assertEq(Core.state.hp, 50)
...
T.describe("Heal history entries", function()
  ...
  T.it("PercentageHeal(75) records the applied 75% maximum HP restoration", function()
    reset()
    Core.SetHP(0, 100)
    Core.PercentageHeal(75)
    local h = Core.state.history[1]
    T.assertEq(h.kind, "PERCENT_HEAL")
    T.assertEq(h.percent, 75)
    T.assertEq(h.applied, 75)
  end)
end)
